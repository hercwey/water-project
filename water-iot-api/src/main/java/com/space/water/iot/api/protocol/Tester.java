...
import static com.space.water.iot.api.protocol.PacketCodec.decodeData;
...
public class Tester {
	...
	private static void unitTestValveControl() {
		...
		valveCmd.setAction(ControlValveType.VALVE_OPEN);

		// [主站] 封装为字节消息
		byte[] packetBytes = encodeData(Protocol.METER_TYPE_10H, "1234567890", "1234", (byte) 0x01, valveCmd);
		...
		// [从站] 封装为字节消息
		byte[] respBytes = encodeData(Protocol.METER_TYPE_10H, "1505900569", "7833", (byte) 0x01, response);
	...
	private static void unitTestReport() {
		...
		// [从站] 封装为字节消息
		byte[] reportBytes = encodeData(Protocol.METER_TYPE_10H, "1505900569", "7833", (byte) 0x02, report);
	...
	private static void unitTestReadConfig() {
		...
		// [主站] 封装为字节消息
		byte[] packetBytes = encodeData(Protocol.METER_TYPE_10H, "1505900569", "7833", (byte) 0x02, command);
		...
		// [从站] 封装为字节消息
		byte[] respBytes = encodeData(Protocol.METER_TYPE_10H, "1505900569", "7833", (byte) 0x01, cmdResp);
		LogUtil.debug("[从站] 发送应答数据:[" + HexUtils.toHexString(packetBytes) + "]");
	...
	private static void unitTestWriteConfig() {
		...
		// 设置表状态字
		meterConfig.setMeterStatusFlag((short) (Protocol.METER_STATUS_MAGNETIC_ON | ~Protocol.METER_STATUS_VALVE_OPEN
		...
		command.setConfigFlag((short) (MeterConfig.FLAG_PERIOD | MeterConfig.FLAG_PERIOD | MeterConfig.FLAG_PERIOD_UNIT
				| MeterConfig.FLAG_MAX_REPORT | MeterConfig.FLAG_EMERG_TIME | MeterConfig.FLAG_VALVE_TIME
				| MeterConfig.FLAG_VALVE_MAINTAIN_TIME | MeterConfig.FLAG_WATER_VLOUME | MeterConfig.FLAG_SAMPLE_UNIT
				| MeterConfig.FLAG_METER_NUM | MeterConfig.FLAG_METER_TIME | MeterConfig.FLAG_METER_STATUS
				| MeterConfig.FLAG_SERVER_IP | MeterConfig.FLAG_SERVER_PORT));
	...
	public static void ST_ReceiveByteBuffer() {
		...
		/* ========== begin of 报文数据 ============ */
		// String msg =
		// "681091002809191100811d1f90009100280919110000002806121906000000023236595d130000002f16";
		// String msg =
		// "681091002809191100811d1f900054360745404352240905010220222222220303358108230000002f16";
		// String msg =
		// "681091002801000105000c070100aaaaaaaa0154360745404307080506010220290089633c7533162f16";
...
//        meterConfig.setServerPort((short) 6538);                        // 设置端口号
	 ...
	 * 单元测试: 读水表开户状态
	 */
	public static void UT_ReadMeterAccount() {
		System.out.println("[主站] 申请读水表开户状态");

		/////////////////////////////////////////////////////////////////////////
		// Begin [主站] 申请读水表开户状态

		// 1. 申请读对象
		MeterAccountReadCmd readCmd = new MeterAccountReadCmd();

		// 2. 封装字节数组
		byte[] packetBytes = PacketCodec.encodeData(Protocol.METER_TYPE_10H, "1505900569", "7833", (byte) 0x02,
				readCmd);
		...
		// -------------------------- Begin 模拟从站操作 ------------------------
		MeterAccountReadResp resp = new MeterAccountReadResp();
		resp.setStatus(MeterAccountReadResp.ACCOUNT_OPEN);
		byte respBytes[] = PacketCodec.encodeData(Protocol.METER_TYPE_10H, "1505900569", "7833", (byte) 0x03, resp);
		// -------------------------- End 模拟从站操作 -------------------------

		/////////////////////////////////////////////////////////////////////////
		// Begin [主站] 接收应答数据
		// [主站] 1. 通过解码器调用解析帧
		PacketFrame packetFrame = PacketCodec.decodeFrame(respBytes);
		...
		if (msgData instanceof MeterAccountReadResp) {
			System.out.println("[主站] 接收 数据体:{" + ((MeterAccountReadResp) msgData).toString() + "}");
		} else {
			System.out.println("[主站] **************** 接收错误 ****************");
		}
		// End [主站] 接收应答数据
		/////////////////////////////////////////////////////////////////////////
	 ...
	 * 单元测试: 写水表开户状态
	 */
	public static void UT_WriteMeterAccount() {
		System.out.println("[主站] 申请读水表开户状态");
		...
		// 1. 申请读对象
		MeterAccountWriteCmd writeCmd = new MeterAccountWriteCmd();
		writeCmd.setAction(MeterAccountWriteCmd.ACCOUNT_CLOSE);
		...
		// -------------------------- Begin 模拟从站操作 ------------------------
		MeterAccountWriteResp resp = new MeterAccountWriteResp();

		byte respBytes[] = PacketCodec.encodeData(Protocol.METER_TYPE_10H, "1505900569", "7833", (byte) 0x04, resp);
		// -------------------------- End 模拟从站操作 -------------------------
		...
		if (msgData instanceof MeterAccountWriteResp) {
			System.out.println("[主站] 接收 数据体:{" + ((MeterAccountWriteResp) msgData).toString() + "}");
		} else {
			System.out.println("[主站] **************** 接收错误 ****************");
		}
		// End [主站] 接收应答数据
