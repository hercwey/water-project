...
public class ByteUtil {
    ...
    public static void setStringIPValue(byte[] address, String ip) {
        String[] sip = ip.split("[.]");

        int AAA = Integer.parseInt(sip[0]) & 0xFF;
        int BBB = Integer.parseInt(sip[1]) & 0xFF;
        int CCC = Integer.parseInt(sip[2]) & 0xFF;
        int DDD = Integer.parseInt(sip[3]) & 0xFF;
    ...
    public static int concatAllLength(byte[] first, byte[]... rest){
        int totalLength = first.length;
        for (byte[] array : rest) {
            if (array != null) {
                totalLength += array.length;
    ...
    public static void setBytes(byte[] bytes, byte data) {
        bytes[0] = (byte) (data & 0xff);
    }
    public static void setBytes(byte[] bytes, short data) {
        bytes[0] = (byte) (data & 0xff);
        bytes[1] = (byte) ((data & 0xff00) >> 8);
    }
    ...
    public static void setBytes(byte[] bytes, char data) {
        bytes[0] = (byte) (data);
        bytes[1] = (byte) (data >> 8);
    }
    ...
    public static void setBytes(byte[] bytes, int data) {
        bytes[0] = (byte) (data & 0xff);
        bytes[1] = (byte) ((data & 0xff00) >> 8);
        bytes[2] = (byte) ((data & 0xff0000) >> 16);
        bytes[3] = (byte) ((data & 0xff000000) >> 24);
    ...
    public static void setBytes(byte[] bytes, float data) {
        int intBits = Float.floatToIntBits(data);
        setBytes(bytes, intBits);
    }
    ...
    public static void setBytes(byte[] bytes, String data) {
    ...
    public static int getInt4Byte(byte[] bytes) {
    ...
    public static int getInt(byte[] bytes){
        if (1 == bytes.length){
            ...
            return (int)getShort(bytes);
        } else if (4 == bytes.length){
            return getInt4Byte(bytes);
        }
        return 0;
    }
