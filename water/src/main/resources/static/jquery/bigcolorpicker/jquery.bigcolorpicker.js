	...
	var bigColorPicker = new function(){
		...
		this.init = function(){
			...
			$(document).bind('mousedown',function(event){
				//console.log("----------mousedown1:"+event.target);
				//console.log("----------mousedown2:"+$(event.target).parents());
				//console.log("----------mousedown3:"+$(event.target).parents().andSelf());
				/*if(!($(event.target).parents().andSelf().is('#bigpicker'))){
					$("#bigpicker").hide();
				}*/
				//srd added at 2019/01/22
		...
		this.showPicker = function(callback,engine,sideLength_){
			...
			if(engine == "L" ){
			    ...
			    bgmage.src = "/jquery/bigcolorpicker/images/big_bgcolor.jpg";
			    //初始化为默认样式
