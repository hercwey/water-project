...
function showPreviewMobile(source, portraitId) {
	var file = source.files[0];
	if (window.FileReader) {
		var fr = new FileReader();
		fr.onloadend = function(e) {
			document.getElementById(portraitId).innerHTML = "<img src='" + e.target.result + "'"+  " width='100%'  height='100%'   >";
			/*document.getElementById(portraitId).innerHTML = "<img src='" + e.target.result + "' height='100px;'>";*/
		};
		fr.readAsDataURL(file);
	}
 ...
 * prompt:提示
 */
function isAllowUploadFile(value, size){
