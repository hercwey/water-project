 ...
 * 		普通发票组件-构造函数
 *	参数:
 * 		@param invoiceId  		发票UI ID(与UI中的ID相对应)
 * 		@param invoiceType  	发票类型  	
 * 								类型值域: special:专用发票  normal:普通发票  参见InvoiceType.js
 * 		invoiceType:  发票类型  			
 *  
 */
function InvoiceNormal(invoiceId,invoiceType) {
	//(1)发票对象
	...
	this.mInvoiceStatus=0;         //发票状态:  0:正常发票   1:作废  2:冲红  2020/01/04
...
InvoiceNormal.prototype.getInvoiceStatus=function(){
	return this.mInvoiceStatus
}
InvoiceNormal.prototype.setInvoiceStatus=function(invoiceStatus){
	this.mInvoiceStatus=invoiceStatus;
}

//显示发票状态图标
//added at 2020/01/04
InvoiceNormal.prototype.displayInvoiceStatus=function(){
 ...
 *    编辑组件事件绑定.
 */
InvoiceNormal.prototype.init = function () {
 ...
 * 初始化发票listener
 */
InvoiceNormal.prototype.initInvoiceListener=function(){
	var self=this;
 ...
 * 初始化各字段编辑器 
 */
InvoiceNormal.prototype.initEditor = function () {
	var self=this;
 ...
 * 功能:初始化购方名称编辑器
 */
InvoiceNormal.prototype.initBuyerNameEditor=function(){
 ...
 * 功能:初始化购方纳税人识别号
 */
InvoiceNormal.prototype.initBuyerTaxNoEditor=function(){
 ...
 * 功能:初始化购方地址及电话
 */
InvoiceNormal.prototype.initBuyerAddressTelEditor=function(){
 ...
 * 购方:银行及帐号
 */
InvoiceNormal.prototype.initBuyerBankAccountNoEditor=function(){
 ...
 * 功能:销方名称编辑器
 */
InvoiceNormal.prototype.initSalerNameEditor=function(){
 ...
 * 功能:初始化销方纳税人识别号编辑器
 */
InvoiceNormal.prototype.initSalerTaxNoEditor=function(){
 ...
 * 功能:初始化销方地址及电话
 */
InvoiceNormal.prototype.initSalerAddressTelEditor=function(){
 ...
 * 购方:银行及帐号
 */
InvoiceNormal.prototype.initSalerBankAccountNoEditor=function(){
 ...
 * 功能:生成初始化参数对象
 */
InvoiceNormal.prototype.genInitParam=function(fieldClass){
 ...
 * 获取发票表头
 */
InvoiceNormal.prototype.getInvoiceHead = function () {
 ...
 */
InvoiceNormal.prototype.setInvoiceHead = function (invoiceHead) {
	this.invoiceHead = invoiceHead;
}

//----------发票附加:invoiceAddition getter and setter-----------
InvoiceNormal.prototype.getInvoiceAddition = function () {
	return this.invoiceAddition;
}
InvoiceNormal.prototype.setInvoiceAddition = function (invoiceAddition) {
 ...
 * 功能:设置发票内容
 */
InvoiceNormal.prototype.setInvoice = function (invoice) {
 ...
 * 注:使用对象中属性
 */
InvoiceNormal.prototype.displayInvoiceHead = function () {
	var self = this;
 ...
 * 功能:显示发票附加信息
 */
InvoiceNormal.prototype.displayInvoiceAddition = function () {
	var self = this;
	const PREFIX = "#" + self.invoiceId + " ";
 ...
 * 功能:	显示发票详情列表
 */
InvoiceNormal.prototype.displayInvoiceDetailList = function () {
 ...
 *                true-删除;  false-不删除 
 */
InvoiceNormal.prototype.removeAllInvoiceDetail = function (removeDataFlag) {
 ...
 * 参数:行索引号
 */
InvoiceNormal.prototype.displayInvoiceDetail = function (rowIndex) {
	var self = this;
 ...
 */
InvoiceNormal.prototype.displayInvoice = function () {
	...
	this.displayInvoiceStatus();  //(4)显示发票状态图标   
}
 ...
 *      价税合计-小写(不在包中)
 */
InvoiceNormal.prototype.calcTaxAndSum = function () {
	...
	for (var i = 0; i < this.invoiceDetailList.length; i++) {
		...
		var dj = InvoiceNormal.toNumber(invoiceDetail.getDJ(), 5, true);  	//条目单价-含税价
		var slv=InvoiceNormal.toNumber(invoiceDetail.getSLV(), 2, true);  //税率
		...
		var sl = InvoiceNormal.toNumber(invoiceDetail.getSL(), 2, true); 	//条目数量		
		var je = InvoiceNormal.toNumber(dj * sl, 2, true);  				//条目金额		
		//console.log("je is:"+je);

		var se = InvoiceNormal.toNumber(sl*priceNoTax * slv, 2, true);   		//条目税额
	...
	*/
	...
	taxSum = InvoiceNormal.toNumber(taxSum, 2, true);  	//格式化
	amountSum = InvoiceNormal.toNumber(amountSum, 2, true)  //格式化
...
InvoiceNormal.prototype.calcInvoiceSum = function () {	
	...
	for (var i = 0; i < this.invoiceDetailList.length; i++) {
		...
		var je = InvoiceNormal.toNumber(invoiceDetail.getJE(), 2, true);  	    	//条目金额		
		//console.log("je is:"+je);

		var se = InvoiceNormal.toNumber(invoiceDetail.getSE(), 2, true);   		//条目税额
 ...
 * 重新计算当前行,所有金额并显示
 */
InvoiceNormal.prototype.recalcTaxAndSumAndDisplay=function(){
	this.calcTaxAndSum();
	this.displayInvoiceAddition();  //(2)显示发票附加信息InvoiceAddition
 ...
 * 参数:str需要判定的字符串
 */
InvoiceNormal.isNull = function (str) {
 ...
 */
InvoiceNormal.toNumber = function (str, fixedLen, roundFlag) {
	var resultValue = 0.00;
	if (InvoiceNormal.isNull(str)) {
		//do nothing
	}
	else {
		var num = Number(str);
 ...
 */
InvoiceNormal.prototype.addInvoiceDetail = function (invoiceDtail) {
 ...
 * 	发票详情条目个数
 */
InvoiceNormal.prototype.getInvoiceDetailNum = function () {
 ...
 * 	功能:  获取发票详情列表.
 */
InvoiceNormal.prototype.getInvoiceDetailList=function(){
 ...
 * 注:	每获取一个后加1 
 */
InvoiceNormal.prototype.getNewRowId = function () {
 ...
 * 
 */
InvoiceNormal.prototype.addRow = function () {
	var self = this;   //保存InvoiceNormal实例
 ...
 * 注:可与上面的函数重构一起重构.
 */
InvoiceNormal.prototype.addHTMLRow = function () {
	var self = this;   //保存InvoiceNormal实例
 ...
 */
InvoiceNormal.prototype.setDetailRowColumnListener=function(rowIndex){
 ...
 */
InvoiceNormal.prototype.setRowSelectorClickListener = function () {
	var self = this;  //发票实例引用.
 ...
 */
InvoiceNormal.prototype.setRowClickListener = function () {
	var self = this;  //发票实例引用.	
 ...
 */
InvoiceNormal.prototype.getRowSelectorStatus = function (rowIndex) {
 ...
 * 		rowIndex  行索引号;
 */
InvoiceNormal.prototype.setRowSelectorStatus = function (rowIndex) {
	const DETAIL_ROW_SELECTOR = "#" + this.invoiceId + " .invoice-detail tbody tr";  //发票详情行选择器
	const ROW_SELECTOR_CLASS = "selected";  //行选择器CLASS
 ...
 * 		rowIndex 行索引号
 */
InvoiceNormal.prototype.setRowStatus = function (rowIndex) {
	const DETAIL_ROW_SELECTOR = "#" + this.invoiceId + " .invoice-detail tbody tr";  //发票详情行选择器
 ...
 * 		追加发票详情条目
 */
InvoiceNormal.prototype.appendEmptyInvoiceDetail = function () {
 ...
 * 		在指定的行索引处插入详情条目
 */
InvoiceNormal.prototype.insertEmptyInvoiceDetail = function (rowIndex) {
	var invoiceDetail = this.genInvoiceDetail();  //生成一张新的发票详情数据
	//在指定的索引处插入发票详情条目对象
 ...
 * 		//生成一张新的发票详情数据
 */
InvoiceNormal.prototype.genInvoiceDetail=function(){
...
InvoiceNormal.prototype.deleteInvoiceDetail = function (rowIndex) {
 ...
 * 注:UI部分
 */
InvoiceNormal.prototype.deleteInvoiceDetailRow = function (rowIndex) {
	const DETAIL_ROW_SELECTOR = "#" + this.invoiceId + " .invoice-detail tbody tr";  //发票详情行选择器	
	var row = $(DETAIL_ROW_SELECTOR)[rowIndex];   //rowIndex--->row
	$(row).remove();
}
 ...
 * 		指定的行对象(JQUERY)	
 */
InvoiceNormal.prototype.getRowJQueryObj = function (rowIndex) {
	const DETAIL_ROW_SELECTOR = "#" + this.invoiceId + " .invoice-detail tbody tr";  //发票详情行选择器
 ...
 * 		rowIndex:行索引号
 */
InvoiceNormal.prototype.setCurrRow = function (rowIndex) {
 ...
 * 	移除所有详情行class:curr
 */
InvoiceNormal.prototype.removeCurrRowStatus = function () {
 ...
 *		无
 */
InvoiceNormal.prototype.removeAllRowSelector = function () {
 ...
 * 	删除行时同时删除对应的商品明细列表对象.
 */
InvoiceNormal.prototype.delRow = function () {
