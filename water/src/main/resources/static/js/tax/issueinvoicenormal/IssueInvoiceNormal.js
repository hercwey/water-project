...
function IssueInvoice(salerInvoiceInfo, loginUserName) {
	...
	//声明发票卷对话框对象  2019/12/20
	this.invoiceVolDialog=new NormalInvoiceVolDialog("invoice-vol-dialog1");
	this.initInvoiceVolDialog();
	...
	//(4)声明普通发票对象
	this.invoiceSpecial = new InvoiceNormal("invoice-normal-1", InvoiceType.FPZL_NORMAL_INVOICE); //初始化专用发票类型
...
//TODO:  2019/12/20 初始发票卷对话框
//
IssueInvoice.prototype.initInvoiceVolDialog=function(){
	var self=this;

	self.invoiceVolDialog.setChoiceCallback(choiceCallback);  	//选择回调函数
	self.invoiceVolDialog.setExitCallback(exitCallback);   		//退出回调函数	
	
	
	//选择发票卷按钮的回调函数(外部业务函数)
	//TODO:  确认发票卷处理函数
	function choiceCallback(respData){		
		console.log("button choice callback function is called!");
		console.log("return data is:"+ JSON.stringify(respData));
		//TODO: *********************2019/12/20 设置发票库存对象

		if(respData==null){
			self.showPopupDialog("未找到发票卷",null);		
		}
		else{
			//(1)保存发票对象		
			console.log("选择发票卷后,保存发票库存对象.");
			console.log("所选择的发票卷是:"+JSON.stringify(respData));
			var inventory=respData;

			self.saveInvoiceInventory(inventory); //保存库存对象  added by hz at 2019/12/15

			//(1.1)设置待开具发票中必要的信息字段
			self.setCurrentInvoiceDM(inventory.FPDM); //设置当前发票的代码
			self.setCurrentInvoiceHM(inventory.FPHM); //设置当前发票的号码				
			...
			//(1.4)显示发票号码确认对话框
			self.setInvoiceHMConfirmDialog(inventory,InvoiceType.FPZL_NORMAL_INVOICE);  //设置发票号码确认对话框中的值
	...
	function exitCallback(){
		console.log("button close vol dialog callback function is called!");		
		self.hideIssueInvoice();  //关闭发票开具界面.
		self.showPopupDialog("点击开具[增值税普通发票]按钮,重新开始",repeatStart);	


		function repeatStart(){
			self.showIssueInvoice();
			self.showInvoiceVolDialog();
		}
...
IssueInvoice.prototype.initPreInvoicePanel = function () {
	...
	function saveInvoiceToBusiness(printInvoiceFlag) {
		...
		invoiceHead.setFPZL(InvoiceType.FPZL_NORMAL_INVOICE); //此处设置发票种类:专用发票
		...
		//更新发票号码及库存
		//TODO: 2019/12/20   更新发票库存.
		function updateInvoiceInventory(){
			var inventory=self.invoiceInventory;  //发票库存

			var invoiceNum=self.preDistInvoiceList.invoiceList.length;	//发票数量		

			if(invoiceNum>0){
				inventory.FPHM= Number(inventory.FPHM) + invoiceNum;
				inventory.KCFPSL=Number(inventory.KCFPSL)-invoiceNum;
			}
		...
		function saveInvoiceCallback(resp) {
			...
			function startPrint() {
				...
				for(var i=0;i<self.preDistInvoiceList.invoiceList.length;i++){
					...
					invoicePrint.setFPZL(InvoiceType.FPZL_NORMAL_INVOICE);
...
IssueInvoice.prototype.resetIssueInvoice=function() {
	...
	//(4)发票号码确认,重新开始流程.
	//TODO 2019/12/20 ------此处需要变更-------继续开具下一张发票
	self.queryInventory(InvoiceType.FPZL_NORMAL_INVOICE);  //查询专用发票号码.
...
IssueInvoice.prototype.initInvoiceHMConfirmDialog = function () {
	...
	function buttonOkCallback() {
		console.log("invoicehm confirm dialog- button ok callback function is called!");
		self.showIssueInvoice(); //显示发票开具界面
	}

	function buttonCancelCallback() {
		console.log("invoicehm confirm dialog-button cancel callback function is called!");
		...
		function repeatStart(){
			self.showIssueInvoice();
			self.showInvoiceVolDialog();  //显示选择发票卷对话框
		}
	}
}
...
IssueInvoice.prototype.queryInventory = function (invoiceType) {
	...
	//生成查询参数
	function genQueryParams(invoiceType){
		//发票类型列表
	...
	var param=genQueryParams(invoiceType);
	$.post(url, param, queryInventoryCallback); //采用load动态加载查询结果

	
	//对比自金税盘查询的库存与前端所保存的库存.
	//如果找到匹配时,则返回true,否则返回false
	//参数:inventoryArr自金税盘查询回来的库存列表
	function compareInventory(inventoryArr){

		var localInventory=self.invoiceInventory;  //本地库存
		var same=false;
		...
		if(inventoryArr==null || inventoryArr.length<=0 ){
			...
			for(var i=0;i<inventoryArr.length;i++){
				var diskInventory=inventoryArr[i];
				if(Number(localInventory.FPHM)==Number(diskInventory.FPHM) &&  
					Number(localInventory.FPDM)==Number(diskInventory.FPDM) && 
					Number(localInventory.FPKCSL)==Number(diskInventory.FPKCSL))
				{
					same=true;
				}
			}
	...
	//	查询库存成功-专用发票-回调函数
	function queryInventoryCallback(resp) {
		self.hideWaitingDailog();  //关闭等待对话框
		...
		if (resp != "") {
			...
			if (inventoryArr.length > 0) {				
				...
				//TODO: 与现有的库存进行对比.如果相等时则继续,否则会提示用户上下不一致.				
				if(compareInventory==false){
					self.showPopupDialog("查询发票库存失败,金税盘端与PC端不一致!", null);
					return;
				}
				
				var inventory = self.invoiceInventory;  //PC端发票库存对象

				console.log("发票库存对象:" + JSON.stringify(inventory));

				//self.saveInvoiceInventory(inventory); //保存库存对象  added by hz at 2019/12/15
				
				self.setInvoiceHMConfirmDialog(inventory,invoiceType);  //设置发票号码确认对话框中的值
				...
				console.log("未获取到库存数据,长度为0!");				
				//util.message("未获取到库存数据!",null,"info");			
				self.showPopupDialog("未获取到库存数据,长度为0!", null);
			}

		} else {
			console.log("未获取到库存数据,发票库存返回为空!");				
...
//------------------设置发票号码确认对话框中的值-------------------
IssueInvoice.prototype.setInvoiceHMConfirmDialog=function(inventory,invoiceType){
	var self=this;

	var invoiceTypeName = InvoiceType.getInvoiceName(invoiceType); //根据发票类型获取发票名称
	//console.log("返回的库存数据是:" + invoiceTypeName + "------" + inventory);
...
/*
	...
	*/
	IssueInvoice.prototype.saveInvoiceInventory= function (inventory) {
		var self=this;
...
//TODO: 2019/12/20 显示发票卷选择对话框
IssueInvoice.prototype.showInvoiceVolDialog=function(){
	this.invoiceVolDialog.show();
}
