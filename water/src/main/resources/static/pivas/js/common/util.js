(function(window) {
	...
	util.loading = function() {
		...
		if(modalobj.length == 0) {
			...
			html = 
				...
				'		<img style="width:100px; height:100px; margin-top:200px;" src="../images/loading.gif" title="正在努力加载...">'+
				'	</div>'+
				'</div>';
			modalobj.html(html);
		}
	...
	util.delConfirm = function(msg, id, callback_name) {
		...
		var content = 
			'			<i class="pull-left fa fa-3x fa-'+icons[type]+'" aria-hidden="true" ></i>'+
	 ...
	 * 删除确认模态框
	 */
	util.cancelConfirm = function(msg, id, callback_name, callback_name2) {
		console.log("del id:"+id);
		if(msg==null || msg==""){
			msg = "确认删除？";
		}
		...
		var content = 
			...
			'			<div class="clearfix"></div>';
		var footer = 
			'			<button type="button" class="btn btn-default" data-dismiss="modal" onclick="javascript:'+callback_name+'(&apos;'+id+'&apos;)'+';">确认撤销</button>' +
			'			<button type="button" class="btn btn-default" data-dismiss="modal" onclick="javascript:'+callback_name2+'(&apos;'+id+'&apos;)'+';">撤销并删除</button>' +
			'			<button type="button" class="btn btn-default" data-dismiss="modal">取消</button>';;
		var modalobj = util.dialog('系统提示', content, footer, {'containerName' : 'modal-del-confirm'});
		modalobj.find('.modal-content').addClass('alert alert-'+type);		
		modalobj.modal('show');
	...
	/*
	 * 确认模态框
	 * 注:在调用此函数时  ok_callback_name cancel_callback_name 需在调用方用引号括起回调函数
	 */
