 ...
 * @Package com.learnbind.ai.common.util.excelutil
 *
 * @Title: ReadMeterExcel.java
 * @Description: 读取表计Excel
 *
 * @author Administrator
 * @date 2019年6月27日 上午9:40:43
 * @version V1.0 
 *
 */
public class ReadMeterExcel {
    
	private static final Log log = LogFactory.getLog(ReadMeterExcel.class);
    ...
    private static Map<String, Object> getMeterParm(XSSFRow xssfRow) {
    	...
    	String newMeterBottom = getValue(xssfRow.getCell(8));//新表表底数
    	String meterUse = getValue(xssfRow.getCell(9));//水表用途
    	String meterType = getValue(xssfRow.getCell(10));//水表类型
    	String meterModel = getValue(xssfRow.getCell(11));//表计型号
    	String readMode = getValue(xssfRow.getCell(12));//抄表方式
    	String channelNo = getValue(xssfRow.getCell(13));//通道号
    	String meterAddress = getValue(xssfRow.getCell(14));//表地址
    	String collectorAddr = getValue(xssfRow.getCell(15));//采集器地址
    	String description = getValue(xssfRow.getCell(16));//描述
    	...
    	meterMap.put("newMeterBottom", newMeterBottom);
    ...
    private static Map<String, Object> getMeterParm(HSSFRow hssfRow) {
    	...
    	String newMeterBottom = getValue(hssfRow.getCell(8));//新表表底数
    	String meterUse = getValue(hssfRow.getCell(9));//水表用途
    	String meterType = getValue(hssfRow.getCell(10));//水表类型
    	String meterModel = getValue(hssfRow.getCell(11));//表计型号
    	String readMode = getValue(hssfRow.getCell(12));//抄表方式
    	String channelNo = getValue(hssfRow.getCell(13));//通道号
    	String meterAddress = getValue(hssfRow.getCell(14));//表地址
    	String collectorAddr = getValue(hssfRow.getCell(15));//采集器地址
    	String description = getValue(hssfRow.getCell(16));//描述
