 ...
 * @Package com.learnbind.ai.common.util.excelutil
 *
 * @Title: ReadPrepaymentDataExcel.java
 * @Description: 读取预存数据
 *
 * @author Administrator
 * @date 2020年1月16日 下午5:10:08
 * @version V1.0 
 *
 */
public class ReadPrepaymentDataExcel {
    
	private static final Log log = LogFactory.getLog(ReadPrepaymentDataExcel.class);
    ...
    private static Map<String, Object> getExcelData(XSSFRow xssfRow) {
    	...
    	String prepayment = getValue(xssfRow.getCell(2));//预存金额
    	
    	Map<String, Object> meterMap = new HashMap<>();
    	meterMap.put("room", room);//门牌号（楼号-单元-房间号）
    	meterMap.put("prepayment", prepayment);//预存金额
    	return meterMap;
    }
    ...
    private static Map<String, Object> getExcelData(HSSFRow hssfRow) {
    	...
    	String prepayment = getValue(hssfRow.getCell(2));//预存金额
