 ...
 * @Package com.learnbind.ai.common.util.excelutil
 *
 * @Title: ReadUpdateCustomerExcel.java
 * @Description: 读取更新客户信息Excel
 *
 * @author Administrator
 * @date 2019年11月8日 下午4:56:07
 * @version V1.0 
 *
 */
public class ReadUpdateCustomerExcel {
    
	private static final Log log = LogFactory.getLog(ReadUpdateCustomerExcel.class);
    ...
    private static Map<String, Object> getMeterParm(XSSFRow xssfRow) {
    	//String room = getValue(xssfRow.getCell(0));//序号
    	String room = getValue(xssfRow.getCell(1));//房间号（楼号-单元-房间号）
    	String preRead = getValue(xssfRow.getCell(2));//上期表底
    	String currRead = getValue(xssfRow.getCell(3));//本期表底
    	//String currRead = getValue(xssfRow.getCell(4));//本期水量
    	String description = getValue(xssfRow.getCell(5));//描述
    	
    	Map<String, Object> meterMap = new HashMap<>();
    	meterMap.put("room", room);//房间号（楼号-单元-房间号）
    	meterMap.put("preRead", preRead);//上期表底
    	meterMap.put("currRead", currRead);//本期表底
    	meterMap.put("description", description);//描述
    	return meterMap;
    }
    ...
    private static Map<String, Object> getMeterParm(HSSFRow hssfRow) {
    	//String room = getValue(hssfRow.getCell(0));//序号
    	String room = getValue(hssfRow.getCell(1));//房间号（楼号-单元-房间号）
    	String preRead = getValue(hssfRow.getCell(2));//上期表底
    	String currRead = getValue(hssfRow.getCell(3));//本期表底
    	//String currRead = getValue(hssfRow.getCell(4));//本期水量
