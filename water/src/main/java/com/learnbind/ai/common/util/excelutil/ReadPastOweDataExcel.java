 ...
 * @Package com.learnbind.ai.common.util.excelutil
 *
 * @Title: ReadPastOweDataExcel.java
 * @Description: 读取往期欠费数据Excel
 *
 * @author Administrator
 * @date 2019年12月10日 下午3:09:56
 * @version V1.0 
 *
 */
public class ReadPastOweDataExcel {
    
	private static final Log log = LogFactory.getLog(ReadPastOweDataExcel.class);
            ...
            for (int rowNum = 1; rowNum <= rowSize; rowNum++) {
                ...
                if (xssfRow != null) {
                	...
                	try {
                		...
                		Map<String, Object> meterMap = getExcelData(xssfRow);
    ...
    /**
     * @Title: getExcelData
     * @Description: 读取Excel数据每行的内容
     * @param xssfRow
     * @return 
     */
    private static Map<String, Object> getExcelData(XSSFRow xssfRow) {
    	//String number = getValue(hssfRow.getCell(0));//序号
    	String room = getValue(xssfRow.getCell(1));//门牌号（楼号-单元-房间号）
    	String oweAmount = getValue(xssfRow.getCell(2));//往期欠费
    	
    	Map<String, Object> meterMap = new HashMap<>();
    	meterMap.put("room", room);//门牌号（楼号-单元-房间号）
    	meterMap.put("oweAmount", oweAmount);//往期欠费
    	return meterMap;
    }
            ...
            for (int rowNum = 1; rowNum <= rowSize; rowNum++) {
                ...
                if (hssfRow != null) {
                	...
                	try {
                		...
                		Map<String, Object> meterMap = getExcelData(hssfRow);//获取表计参数
                		meterMapList.add(meterMap);
    ...
    /**
     * @Title: getExcelData
     * @Description: 读取Excel每行内容
     * @param hssfRow
     * @return 
     */
    private static Map<String, Object> getExcelData(HSSFRow hssfRow) {
    	//String number = getValue(hssfRow.getCell(0));//序号
    	String room = getValue(hssfRow.getCell(1));//门牌号（楼号-单元-房间号）
    	String oweAmount = getValue(hssfRow.getCell(2));//往期欠费
