 ...
 * @Package com.learnbind.ai.common.util.excelutil
 *
 * @Title: ReadEgMeterExcel.java
 * @Description: 读取旧系统艺高excel
 *
 * @author Administrator
 * @date 2019年9月25日 下午1:09:09
 * @version V1.0 
 *
 */
public class ReadEgMeterExcel {
    
	private static final Log log = LogFactory.getLog(ReadEgMeterExcel.class);
    ...
    private static TestMeterAddrBean getMeterParm(XSSFRow xssfRow) {
    	...
    	String room = getValue(xssfRow.getCell(2));//地理位置名称（楼号-单元-房间号）
    	
    	String meterAddr = getValue(xssfRow.getCell(3));//表地址
    ...
    private static TestMeterAddrBean getMeterParm(HSSFRow hssfRow) {
    	...
    	String room = getValue(hssfRow.getCell(2));//地理位置名称（楼号-单元-房间号）
    	
    	String meterAddr = getValue(hssfRow.getCell(3));//表地址
