 ...
 *
 * @Title: ControlValveProducer.java
 * @Description: 生产者-下发控制设备（开关阀控制）命令
 *
 * @author SRD
 ...
 */
@Component
public class ControlValveProducer {

	/**
	 * @Fields log：日志
	 */
	private static final Logger log = LoggerFactory.getLogger(ControlValveProducer.class);
	 ...
	 * 通过构造函数 实例化对象
	 */
	public ControlValveProducer() throws MQClientException {
		
		String producerGroup = MQConstant.P_G_SORTH_CONTROL_VALVE;//生产者分组
		//示例生产者
	...
	public SendResult sendMsg(String keys, String body) throws MQClientException, RemotingException, MQBrokerException, InterruptedException {
		...
		String tag = rocketTopicConfig.getTagControlValveSouth();// tag
