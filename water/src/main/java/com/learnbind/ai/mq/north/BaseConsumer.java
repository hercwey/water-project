 ...
 * @Package com.learnbind.ai.mq.north
 *
 * @Title: BaseConsumer.java
 * @Description: 消费者抽象类
 *
 * @author SRD
 * @date 2020年3月5日 下午11:24:49
 * @version V1.0 
 *
 */
@Component
...
//			String tag = rocketTopicConfig.getTagAccountStatusReadNorth();// tag
				...
				public ConsumeConcurrentlyStatus consumeMessage(List<MessageExt> msgs,
					...
					try {
						for (Message msg : msgs) {
							...
							//log.info("消费者分组-读开户状态返回数据【" + consumerGroup + "】，主题topic【" + msg.getTopic() + "】，tag【" + tag + "】，消费消息【" + body + "】");
							log.info("----------------------------------------------------------------------");
							log.info("| Consumer接收消息");
							log.info("| Topic:"+msg.getTopic());
							log.info("| Tag:"+msg.getTags());
							log.info("| Data:"+body);
							log.info("----------------------------------------------------------------------");
							processMessage(body);
						}
			...
			consumer.start();
			log.info("----------------------------------------------------------------------");
			log.info("| 消费者 "+this.getClass().getName()+" 启动成功----------");
			log.info("----------------------------------------------------------------------");
