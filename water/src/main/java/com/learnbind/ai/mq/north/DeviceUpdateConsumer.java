...
public class DeviceUpdateConsumer {
	...
	public DeviceUpdateConsumer() throws MQClientException {
		try {
			...
			String consumerGroup = MQConstant.C_G_NORTH_DEVICE_UPDATE;// 消费者分组
			String topicName = rocketTopicConfig.getTopicName();// 主题名称
			String tag = rocketTopicConfig.getTagDeviceUpdateNorth();// tag
				...
				public ConsumeConcurrentlyStatus consumeMessage(List<MessageExt> msgs,
					...
					try {
						for (Message msg : msgs) {
							...
							log.debug("消费者分组-修改设备返回数据【" + consumerGroup + "】，主题topic【" + msg.getTopic() + "】，tag【" + tag
									+ "】，消费消息【" + body + "】");
						}
