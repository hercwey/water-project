 ...
 *
 * @Title: QueryParmsProducer.java
 * @Description: 生产者-下发查询读表配置参数数据命令
 *
 * @author SRD
 ...
 */
@Component
public class QueryParmsProducer {

	/**
	 * @Fields log：日志
	 */
	private static final Logger log = LoggerFactory.getLogger(QueryParmsProducer.class);
	 ...
	 * 通过构造函数 实例化对象
	 */
	public QueryParmsProducer() throws MQClientException {
		
		String producerGroup = MQConstant.P_G_SORTH_QUERY_PARMS;//生产者分组
		//示例生产者
	...
	public SendResult sendMsg(String keys, String body) throws MQClientException, RemotingException, MQBrokerException, InterruptedException {
		...
		String tag = rocketTopicConfig.getTagQueryParmsSouth();// tag
        ...
        log.debug("下发查询读表配置参数数据命令到MQ，发送结果是："+sendResult);
        return sendResult;
	}
