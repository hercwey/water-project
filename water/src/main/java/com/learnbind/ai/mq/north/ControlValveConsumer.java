...
public class ControlValveConsumer {
	...
	public void start() throws MQClientException {
		try {
			...
			String consumerGroup = MQConstant.C_G_NORTH_CONTROL_VALVE;// 消费者分组
			String topicName = rocketTopicConfig.getTopicName();// 主题名称
			String tag = rocketTopicConfig.getTagControlValveNorth();// tag
				...
				public ConsumeConcurrentlyStatus consumeMessage(List<MessageExt> msgs,
					...
					try {
						for (Message msg : msgs) {
							...
							log.info("消费者分组-控制设备（开关阀控制）返回数据【" + consumerGroup + "】，主题topic【" + msg.getTopic() + "】，tag【" + tag
									+ "】，消费消息【" + body + "】");
			...
			consumer.start();
			log.info("----------消费者-监听控制设备（开关阀控制）返回数据启动成功");
