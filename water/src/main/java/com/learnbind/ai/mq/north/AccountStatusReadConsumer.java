 ...
 */
@Component
public class AccountStatusReadConsumer {
	...
	public void start() throws MQClientException {
		try {
			...
			String consumerGroup = MQConstant.C_G_NORTH_ACCOUNT_STATUS_READ;// 消费者分组
			String topicName = rocketTopicConfig.getTopicName();// 主题名称
			String tag = rocketTopicConfig.getTagAccountStatusReadNorth();// tag
				...
				public ConsumeConcurrentlyStatus consumeMessage(List<MessageExt> msgs,
					...
					try {
						for (Message msg : msgs) {
							...
							log.info("消费者分组-读开户状态返回数据【" + consumerGroup + "】，主题topic【" + msg.getTopic() + "】，tag【" + tag
									+ "】，消费消息【" + body + "】");
							AccountStatusReadResponse statusReadRsp = AccountStatusReadResponse.fromJson(body);
