...
public class ConfigThresholdConsumer {
	...
	public void start() throws MQClientException {
		try {
			...
			String consumerGroup = MQConstant.C_G_NORTH_CONFIG_THRESHOLD;// 消费者分组
			String topicName = rocketTopicConfig.getTopicName();// 主题名称
			String tag = rocketTopicConfig.getTagConfigThresholdNorth();// tag
				...
				public ConsumeConcurrentlyStatus consumeMessage(List<MessageExt> msgs,
					...
					try {
						for (Message msg : msgs) {
							...
							log.info("消费者分组-设置阈值返回数据【" + consumerGroup + "】，主题topic【" + msg.getTopic() + "】，tag【" + tag
									+ "】，消费消息【" + body + "】");
