 ...
 * @Package com.learnbind.ai.mq.south
 *
 * @Title: AccountStatusReadProducer.java
 * @Description: 生产者-下发读表开户状态命令
 *
 * @author SRD
 * @date 2020年3月3日 下午11:58:43
 * @version V1.0 
 *
 */
@Component
public class AccountStatusReadProducer {

	/**
	 * @Fields log：日志
	 */
	private static final Logger log = LoggerFactory.getLogger(AccountStatusReadProducer.class);
	 ...
	 * 通过构造函数 实例化对象
	 */
	public AccountStatusReadProducer() throws MQClientException {
		
		String producerGroup = MQConstant.P_G_SORTH_ACCOUNT_STATUS_READ;//生产者分组
		//示例生产者
	...
	public SendResult sendMsg(String keys, String body) throws MQClientException, RemotingException, MQBrokerException, InterruptedException {
		...
		String tag = rocketTopicConfig.getTagAccountStatusReadSouth();// tag
        ...
        log.info("下发读表开户状态命令到MQ，发送结果是："+sendResult);
        return sendResult;
	}
