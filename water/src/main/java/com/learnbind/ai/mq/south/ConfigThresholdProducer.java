 ...
 *
 * @Title: ConfigThresholdProducer.java
 * @Description: 生产者-下发设置阈值命令
 *
 * @author SRD
 ...
 */
@Component
public class ConfigThresholdProducer {

	/**
	 * @Fields log：日志
	 */
	private static final Logger log = LoggerFactory.getLogger(ConfigThresholdProducer.class);
	 ...
	 * 通过构造函数 实例化对象
	 */
	public ConfigThresholdProducer() throws MQClientException {
		
		String producerGroup = MQConstant.P_G_SORTH_CONFIG_THRESHOLD;//生产者分组
		//示例生产者
	...
	public SendResult sendMsg(String keys, String body) throws MQClientException, RemotingException, MQBrokerException, InterruptedException {
		...
		String tag = rocketTopicConfig.getTagConfigThresholdSouth();// tag
        ...
        log.debug("下发设置阈值命令到MQ，发送结果是："+sendResult);
        return sendResult;
	}
