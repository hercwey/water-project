 ...
 *
 * @Title: DeviceQueryProducer.java
 * @Description: 生产者-调用IOT电信平台查询设备接口
 *
 * @author SRD
...
//@Component
@AutoConfigureAfter
public class DeviceQueryProducer {

	/**
	 * @Fields log：日志
	 */
	private static final Logger log = LoggerFactory.getLogger(DeviceQueryProducer.class);
	 ...
	 * 通过构造函数 实例化对象
	 */
	public DeviceQueryProducer() throws MQClientException {
		
		String producerGroup = MQConstant.P_G_SORTH_DEVICE_QUERY;//生产者分组
		//示例生产者
	...
	public SendResult sendMsg(String keys, String body) throws MQClientException, RemotingException, MQBrokerException, InterruptedException {
		...
		String tag = rocketTopicConfig.getTagDeviceQuerySouth();// tag
        ...
        log.debug("调用IOT电信平台查询设备接口到MQ，发送结果是："+sendResult);
        return sendResult;
	}
