 ...
 * @Package com.learnbind.ai.mq.south
 *
 * @Title: AccountStatusWriteProducer.java
 * @Description: 生产者-下发写表开户状态命令
 *
 * @author SRD
 * @date 2020年3月8日 上午12:06:25
 * @version V1.0 
 *
 */
@Component
public class AccountStatusWriteProducer {

	/**
	 * @Fields log：日志
	 */
	private static final Logger log = LoggerFactory.getLogger(AccountStatusWriteProducer.class);
	 ...
	 * 通过构造函数 实例化对象
	 */
	public AccountStatusWriteProducer() throws MQClientException {
		
		String producerGroup = MQConstant.P_G_SORTH_ACCOUNT_STATUS_WRITE;//生产者分组
		//示例生产者
	...
	public SendResult sendMsg(String keys, String body) throws MQClientException, RemotingException, MQBrokerException, InterruptedException {
		...
		String tag = rocketTopicConfig.getTagAccountStatusWriteSouth();// tag
        ...
        log.info("下发写表开户状态命令到MQ，发送结果是："+sendResult);
        return sendResult;
	}
