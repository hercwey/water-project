 ...
 * @date 2019年11月1日 上午9:17:03
 * @version V1.0 
 *
 */
@Controller
@RequestMapping(value = "/water-fee-stat")
public class WaterFeeStatController {
	private static Log log = LogFactory.getLog(OweFeeStatController.class);
	private static final String TEMPLATE_PATH = "statistic/fee/waterfee/"; // 页面目录
	...
	public String table(Model model, Integer pageNum, Integer pageSize, String period, String traceIds, Integer settlementStatus, Integer accountStatus, String searchCond, String startDate, String endDate) {
		...
		List<Map<String, Object>> accountItemMapList = statisticService.getListGroupByPriceCode(traceIds, period, startDate, endDate);
		PageInfo<Map<String, Object>> pageInfo = new PageInfo<>(accountItemMapList);// (使用了拦截器或是AOP进行查询的再次处理)

//		accountItemMapList中每个对象的属性：
//		PW.PRICE_CODE,											price_code
//		MIN(PW_LOG.LADDER_NAME) AS PRICE_NAME,					水价名称
//		MIN(PW.BASE_PRICE) AS BASE_PRICE,						基础水价
//		MIN(PW.TREATMENT_FEE) AS TREATMETN_FEE,					污水处理费
//		SUM( I.CREDIT_AMOUNT ) AS PAYABLE_AMOUNT,				应缴金额
//		SUM( I.DEBIT_AMOUNT ) AS PAID_AMOUNT,					已缴金额
//		SUM( I.CREDIT_AMOUNT - I.DEBIT_AMOUNT ) AS OWE_AMOUNT	欠费金额
		...
		for(Map<String, Object> map : accountItemMapList) {
			...
			String oweAmount = map.get("OWE_AMOUNT").toString();
			BigDecimal oweAmountBd = new BigDecimal(oweAmount);
			oweTotalAmount = BigDecimalUtils.add(oweTotalAmount, oweAmountBd);
		}
		// 传递如下数据至前台页面
