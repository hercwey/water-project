 ...
 */
@Controller
@RequestMapping(value = "/meter-receive-record")
public class MeterReceiveRecordController {
	private static Log log = LogFactory.getLog(MeterReceiveRecordController.class);
	private static final String TEMPLATE_PATH = "meter_stock/meter_receive_record/"; // 页面
	private static final String INSTALL_TEMPLATE_PATH = "meter_stock/meter_receive_record/install_receipt/"; // 页面
	private static final String CHANGE_TEMPLATE_PATH = "meter_stock/meter_receive_record/change_receipt/"; // 页面
	...
	public String table(Model model, Integer pageNum, Integer pageSize, String searchCond, String traceIds) {
		...
		for(Meters meter : meterList) {
			...
			meterMap.put("checkDateStr", meter.getCheckDateStr());
	...
	private List<Integer> getCycleStatus(){
		...
		//待用
		Integer inactiveStatus = EnumMeterCycleStatus.RECEIVE.getValue();
	 ...
	 * @throws Exception 
	 */
	@RequestMapping(value = "/cancel-receive", produces = "application/json")
	@ResponseBody
	public Object cancelRecive(Long meterId) throws Exception {
		Meters meter = metersService.selectByPrimaryKey(meterId);
		//将水表状态修改为未用
		meter.setCycleStatus(EnumMeterCycleStatus.INACTIVE.getValue());
		int rows = metersService.updateByPrimaryKeySelective(meter);
		if(rows > 0) {
			//添加库存日志
			meterStockTraceService.insertTrace(meter.getId(), EnumMeterStockStatus.CANCEL_RECEIVE.getValue());
