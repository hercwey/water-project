package com.learnbind.ai.controller.statistic.readmeter.yearrate;
...
public class ReadMeterYearRateController {
	private static Log log = LogFactory.getLog(ReadMeterYearRateController.class);
	private static final String TEMPLATE_PATH = "statistic/readmeter/yearrate/"; // 页面目录
	 ...
	 * @return 
	 */
	@RequestMapping(value = "/readmeter/yearrate/main")
	public String statisticReadMeterYearRateMain(Model model) {
		return TEMPLATE_PATH + "statistic_readmeter_year_rate_main";
	}
	 ...
	 * 			]
	 */
	@RequestMapping(value = "/readmeter/yearrate/table")
	public String statisticReadMeterYearRateTable(Integer pageNum, 
											  Integer pageSize,
											  String searchCond, 
											  Model model) {
		...
		List<String> itemValueList=statReadMeterYearRate(period, traceIds);  //统计抄表率.  统计需抄表数量,统计实际抄表数量
		...
		return TEMPLATE_PATH+"statistic_readmeter_year_rate_table";
	}
	 ...
	 * 		[应抄数,  实抄数, 未抄数,抄表率 ]
	 */
	private List<String> statReadMeterYearRate(String period, String traceIds){
		
		int allAmount = this.getAllMeterAmount(period, traceIds);//应抄表数
		int readMeterAmount=statisticService.statHasReadMeterYearAmount(period, traceIds);  	//实抄表数
		
		
		List<String>  itemValueList=new ArrayList<>();		
		itemValueList.add(Integer.toString(allAmount));   				//全部
		itemValueList.add(Integer.toString(readMeterAmount));  				//己抄
		itemValueList.add(Integer.toString(allAmount-readMeterAmount));  	//未抄数量
		float readRate=0;
		if(allAmount != 0) {
			readRate=(float)readMeterAmount/allAmount*100;  //抄表率
		}
	...
	/**
	 * @Title: getAllMeterAmount
	 * @Description: 获取应抄表数
	 * @param period
	 * @param traceIds
	 * @return 
	 */
	private int getAllMeterAmount(String period, String traceIds) {
		int total = 0;
		if(StringUtils.isBlank(traceIds)) {
			List<Location> locationList = locationService.getBlockListByPid(null);
			for(Location location : locationList) {
				int periodAmount =statisticService.statNeedReadMeterYear(period, location.getTraceIds());  			//应抄表数
				int meterAmount = statisticService.statNeedReadMeterAmount(location.getTraceIds());
				int num = periodAmount * meterAmount;
				total = num+ total;
			}
			
		} else {
			int periodAmount =statisticService.statNeedReadMeterYear(period, traceIds);  			//应抄表数
			int meterAmount = statisticService.statNeedReadMeterAmount(traceIds);
			total = meterAmount*periodAmount;
		}
		return total;
	}
	/**
	 ...
	 * @return 
	 */
	@RequestMapping(value = "/readmeter/yearrate/chart")
	public String statisticReadMeterYearChart(String searchCond, Model model) {			
		...
		return TEMPLATE_PATH + "statistic_readmeter_year_rate_pie";
	}
	...
	private List<Map<String, Object>> statReadMeterRateByProperty(String period,List<String> itemNameList, String traceIds) {
		...
		int meterAmount=this.getAllMeterAmount(period, traceIds);  			//应抄表数
		int readMeterAmount=statisticService.statHasReadMeterYearAmount(period, traceIds);  	//实抄表数
