package com.learnbind.ai.controller.statistic.fee.actualstat;
 ...
 * @Description: 实收统计(用户查询指定时间段内实收金额)
 *
 * @author lenovo
...
public class ActualStatController {
	private static Log log = LogFactory.getLog(ActualStatController.class);
	private static final String TEMPLATE_PATH = "statistic/fee/actualstat/"; // 页面目录
	...
	//------------------实收汇总,指定时间段内---------------------
		...
		@RequestMapping(value = "/fee/actualstat/main")
		public String statisticFeeReceStatMain(Model model) {
			return TEMPLATE_PATH + "statistic_fee_actualstat_main";
		 ...
		 * @return 
		 */
		@RequestMapping(value = "/fee/actualstat/table")
		public String statisticFeeReceStatTable(Integer pageNum, 
			...
			accountItemList=statisticService.searchActualReceList(periodStart,periodEnd, traceIds);  //查询指定期间的实收数据列表
			PageInfo<Map<String,Object>> pageInfo = new PageInfo<>(accountItemList); 	   // (使用了拦截器或是AOP进行查询的再次处理)
			
			
		
			List<Map<String, Object>> accountItemSumList = statisticService.statReceivableSum(periodStart, periodEnd, traceIds);
			// 查询合计
			...
			return TEMPLATE_PATH+"statistic_fee_actualstat_table";
		}
		 ...
		 * @return 
		 */
		@RequestMapping(value = "/fee/actualstat/chart")
