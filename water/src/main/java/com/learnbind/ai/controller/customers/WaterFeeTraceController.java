 ...
 */
@Controller
@RequestMapping(value = "/water-fee-trace")
public class WaterFeeTraceController {
	
	private static Log log = LogFactory.getLog(WaterFeeTraceController.class);
	
	private static final String TEMPLATE_PATH = "customers/water_fee_trace/"; // 页面目录角色
	...
	public String starter(Model model) {
		return TEMPLATE_PATH + "water_fee_trace_starter";
	}
	...
	public String main(Model model) {
		...
		return TEMPLATE_PATH + "water_fee_trace_main";
	}
	...
	public String table(Model model, Integer pageNum, Integer pageSize, String searchCond) {
		...
		List<DiscountWaterFeeTrace> traceList = discountWaterFeeTraceService.search(searchCond);
		PageInfo<List<DiscountWaterFeeTrace>> pageInfo = new PageInfo(traceList);// (使用了拦截器或是AOP进行查询的再次处理)
		
		List<Map<String, Object>> customerMapList = new ArrayList<>();
		for(DiscountWaterFeeTrace ft : traceList) {
		...
		return TEMPLATE_PATH + "water_fee_trace_table";
	}
